# This script reduces the size of the data table, calculates probabilities 
# before (pre-processing) and uses SBO for lower-order n-grams
...
setkey(ngrams, ngram, tail)

# Remove n-grams with a count of 3 or less
ngrams <- ngrams[count > 2]

# Calculate MLE probabilities for each n-gram
#ngrams <- ngrams[!(n == 1), agg := sum(count), by = ngram]
#ngrams <- ngrams[n == 1, agg := sum(count), by = ngram]
#ngrams <- ngrams[n == 6, prob := count / agg]
#ngrams <- ngrams[n == 5, prob := (count / agg) * .4]
#ngrams <- ngrams[n == 4, prob := (count / agg) * .4 * .4]
#ngrams <- ngrams[n == 3, prob := (count / agg) * .4 * .4 * .4]
#ngrams <- ngrams[n == 2, prob := (count / agg) * .4 * .4 * .4 * .4]
#ngrams <- ngrams[n == 1, prob := (count / agg) * .4 * .4 * .4 * .4 * .4]
#ngrams <- ngrams[, agg := NULL]
...
myPredictionFunction <- compiler::cmpfun(function(x){
        ...
        if(nrow(fivegrams) > 0) {
                denom <- fivegrams[, .(sum(count))][1, 1]
                fivegrams <- fivegrams[, prob := ((count / denom[[1]]) * .4)]
                # Note observed fivegram tail words
                observedTails <- c(observedTails, fivegrams$tail)
        }
        ...
        if(nrow(quadgrams) > 0) {
                denom <- quadgrams[, .(sum(count))][1, 1]
                quadgrams <- quadgrams[, prob := ((count / denom[[1]]) * .4 * .4)]
                # Note observed quadgram tail words
                observedTails <- c(observedTails, quadgrams$tail)
        }
        ...
        if(nrow(trigrams) > 0) {
                denom <- trigrams[, .(sum(count))][1, 1]
                trigrams <- trigrams[, prob := ((count / denom[[1]]) * .4 * .4 * .4)]
                # Note observed trigram tail words
                observedTails <- c(observedTails, trigrams$tail)
        }
        ...
        if(nrow(bigrams) > 0) {
                denom <- bigrams[, .(sum(count))][1, 1]
                bigrams <- bigrams[, prob := ((count / denom[[1]]) * .4 * .4 * .4 * .4)]
                # Note observed bigram tail words
                observedTails <- c(observedTails, bigrams$tail)
        }
        ...
        # Calculate MLEs
        if(denom > 0) {
                unigrams <- unigrams[, prob := ((count / denom[[1]])) * .4 * .4 * .4 * .4 * .4]
        }
